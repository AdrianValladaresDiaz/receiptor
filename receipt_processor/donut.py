...
def runDONUT(inQ: Queue, outQ: Queue, process_counter: multiprocessingValueAlias):
    ...
    try:
        ...
        while True:
            if inQ.qsize()>0:
                img = inQ.get()
                img_info = apply_donut_to_image(img)
        ...
        logger.error(f'PROCESS EXIT DUE TO ERROR: {str(e.with_traceback(None))}')
        with process_counter.get_lock():
            process_counter.value -= 1
